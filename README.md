...
`llava:latest` が表示されればOK。表示されない場合は手動でpull：

```bash
curl http://ollama:11434/api/pull -d '{"name":"llava"}'
```
...
```bash
# 通常: 撮影 → マーカー合成 → Ollama(llava)分析 を一気通貫
...
| `--analysis-model` | `llava:latest` | 使用モデル |
| `--analysis-timeout` | `120` | タイムアウト秒数 |
| `--analysis-prompt` | 組み込みプロンプト | カスタムプロンプト文字列 |
...
`export_prompt.py` の `estimate_pnl()` はロング・ショートを別スタックでFIFO管理します。
...
**Ollamaに繋がらない（404エラー）**
llavaがまだダウンロードされていない可能性があります：
```bash
curl http://ollama:11434/api/tags                              # モデル一覧確認
curl http://ollama:11434/api/pull -d '{"name":"llava"}'        # 手動pull
```

**損益が0円と表示される**
`export_prompt.py` を使ってください。`batch_snapshot.py` の分析はOllamaに渡すテキストの生成のみで、損益計算は `export_prompt.py` の `estimate_pnl()` が担います。